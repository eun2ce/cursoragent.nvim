--- Diff module for Cursor Agent Neovim integration.
...
local function get_autocmd_group()
  if not autocmd_group then
    autocmd_group = vim.api.nvim_create_augroup("CursorAgentMCPDiff", { clear = true })
  end
...
---Find the Cursor Agent terminal window to keep focus there.
---Uses the terminal provider to get the active terminal buffer, then finds its window.
---@return number? win_id Window ID of the Cursor Agent terminal window, or nil if not found
local function find_cursoragent_terminal_window()
...
---Display existing Cursor Agent terminal in new tab
...
local function display_terminal_in_new_tab()
  ...
  local existing_terminal_win = find_cursoragent_terminal_window()
  ...
  vim.api.nvim_create_autocmd("BufEnter", {
    ...
    desc = "Auto-enter terminal mode when focusing Cursor Agent terminal",
  })
...
---@param user_config CursorAgentConfig The configuration passed from init.lua
...
local function create_temp_file(content, filename)
  local base_dir_cache = vim.fn.stdpath("cache") .. "/cursoragent_diffs"
  ...
  if mkdir_ok_cache then
    ...
    local base_dir_temp = vim.fn.stdpath("cache") .. "/cursoragent_diffs_fallback"
  ...
  if session_id == "" then -- Fallback if all characters were problematic, ensuring a directory can be made.
    session_id = "cursoragent_session"
  end
...
local function cleanup_temp_file(tmp_file)
  if tmp_file and vim.fn.filereadable(tmp_file) == 1 then
    ...
    if vim.fs and type(vim.fs.remove) == "function" then
      ...
      if not ok_file then
        vim.notify(
          "CursorAgent: Error removing temp file " .. tmp_file .. ": " .. tostring(err_file),
          vim.log.levels.WARN
        )
      end
      ...
      if not ok_dir then
        vim.notify(
          "CursorAgent: Error removing temp directory " .. tmp_dir .. ": " .. tostring(err_dir),
          vim.log.levels.INFO
        )
      end
    else
      ...
      vim.notify(
        "CursorAgent: Cannot perform standard cleanup: "
          .. reason
          .. ". Affected file: "
          .. tmp_file
      ...
      if not os_ok then
        vim.notify(
          "CursorAgent: Fallback os.remove also failed for file " .. tmp_file .. ": " .. tostring(os_err),
          vim.log.levels.ERROR
        )
      end
    end
...
local function setup_new_buffer(
  ...
  vim.b[new_buf].cursoragent_diff_tab_name = tab_name
  vim.b[new_buf].cursoragent_diff_new_win = new_win
  vim.b[new_buf].cursoragent_diff_target_win = target_win_for_meta
  ...
  if config and config.diff_opts and config.diff_opts.keep_terminal_focus then
    vim.schedule(function()
      ...
      local terminal_win = find_cursoragent_terminal_window()
...
function M._open_native_diff(old_file_path, new_file_path, new_file_contents, tab_name)
  ...
  local cleanup_group = vim.api.nvim_create_augroup("CursorAgentDiffCleanup", { clear = false })
...
function M.open_diff_blocking(old_file_path, new_file_path, new_file_contents, tab_name)
  ...
  local success, err = pcall(M._setup_blocking_diff, {
    ...
    if resume_success then
      ...
      if _G.cursoragent_deferred_responses and _G.cursoragent_deferred_responses[co_key] then
        _G.cursoragent_deferred_responses[co_key](resume_result)
        _G.cursoragent_deferred_responses[co_key] = nil
      else
      ...
      local co_key = tostring(co)
      if _G.cursoragent_deferred_responses and _G.cursoragent_deferred_responses[co_key] then
        _G.cursoragent_deferred_responses[co_key]({
          error = {
            code = -32603,
...
function M.close_diff_by_tab_name(tab_name)
  ...
  if diff_data.status == "saved" then
    -- Cursor Agent CLI has written the file, reload any open buffers
...
function M.accept_current_diff()
  ...
  local tab_name = vim.b[current_buffer].cursoragent_diff_tab_name
