---@brief [[
--- Lock file management for Cursor Agent Neovim integration.
--- This module handles creation, removal and updating of lock files
--- which allow Cursor Agent CLI to discover the Neovim integration.
---@brief ]]
---@module 'cursoragent.lockfile'
local M = {}

---Path to the lock file directory
---@return string lock_dir The path to the lock file directory
local function get_lock_dir()
  local cursor_config_dir = os.getenv("CURSOR_CONFIG_DIR")
  if cursor_config_dir and cursor_config_dir ~= "" then
    return vim.fn.expand(cursor_config_dir .. "/ide")
  else
    return vim.fn.expand("~/.cursor/ide")
  end
end
