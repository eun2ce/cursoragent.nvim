---@brief Centralized logger for Cursor Agent Neovim integration.
-- Provides level-based logging.
---@module 'cursoragent.logger'
local M = {}
...
---@param plugin_config CursorAgentConfig The configuration table (e.g., from cursoragent.init.state.config).
function M.setup(plugin_config)
  ...
  if conf and conf.log_level and level_values[conf.log_level] then
    ...
    vim.notify(
      "CursorAgent Logger: Invalid or missing log_level in configuration (received: "
...
local function log(level, component, message_parts)
  ...
  vim.schedule(function()
    if level == M.levels.ERROR then
      vim.notify(prefix .. " " .. message, vim.log.levels.ERROR, { title = "CursorAgent Error" })
    elseif level == M.levels.WARN then
      vim.notify(prefix .. " " .. message, vim.log.levels.WARN, { title = "CursorAgent Warning" })
    else
...
---@param level_name CursorAgentLogLevel The level name ("error", "warn", "info", "debug", "trace")
