-- Tool implementation for Cursor Agent Neovim integration
local M = {}
...
function M.register_all()
  ...
  M.register(require("cursoragent.tools.open_file"))
  M.register(require("cursoragent.tools.get_current_selection"))
  M.register(require("cursoragent.tools.get_open_editors"))
  M.register(require("cursoragent.tools.open_diff"))
  M.register(require("cursoragent.tools.get_latest_selection"))
  M.register(require("cursoragent.tools.close_all_diff_tabs"))
  M.register(require("cursoragent.tools.get_diagnostics"))
  M.register(require("cursoragent.tools.get_workspace_folders"))
  M.register(require("cursoragent.tools.check_document_dirty"))
  M.register(require("cursoragent.tools.save_document"))

  -- Register internal tools without schemas (not exposed via MCP)
  M.register(require("cursoragent.tools.close_tab"))
end
...
function M.register(tool_module)
  if not tool_module or not tool_module.name or not tool_module.handler then
    ...
    vim.notify(
      ...
      { title = "CursorAgent Tool Registration" }
    )
    return
  end
...
function M.handle_invoke(client, params) -- client needed for blocking tools
  ...
  if tool_data.requires_coroutine then
    ...
    require("cursoragent.logger").debug("tools", "Wrapping " .. tool_name .. " in coroutine for blocking behavior")
    local co = coroutine.create(function()
      return tool_data.handler(input)
    end)

    require("cursoragent.logger").debug("tools", "About to resume coroutine for " .. tool_name)
    ...
    if coroutine.status(co) == "suspended" then
      require("cursoragent.logger").debug("tools", "Coroutine is suspended - tool is blocking, will respond later")
